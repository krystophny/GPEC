...
c-----------------------------------------------------------------------
      IF(diagnose)THEN
         ...
         CALL ascii_open(debug_unit,"gsec.out","UNKNOWN")
         CALL bin_open(bin_unit,"gsec.bin","UNKNOWN","REWIND","none")
         CALL spline_write1(eq,.TRUE.,.TRUE.,debug_unit,bin_unit,
     $        .FALSE.)
...
c     declarations.
c-----------------------------------------------------------------------
      SUBROUTINE gsec_int(u1max,mstep,final,eq)
...
c-----------------------------------------------------------------------
      DO
         IF(final)THEN
            CALL gsec_prof(psi,u,pfac,p1fac,jfac)
            eq%xs(istep)=psi
            f=u(3)
            p=pfac*beta0/2
...
c-----------------------------------------------------------------------
         ...
         istep=istep+1
         CALL lsode(gsec_der,neq,u,psi,psiout,itol,rtol,atol,itask,
...
c     declarations.
c-----------------------------------------------------------------------
      SUBROUTINE gsec_der(neq,psi,u,du)
...
c     declarations.
c-----------------------------------------------------------------------
      SUBROUTINE gsec_prof(psi,u,pfac,p1fac,jfac)
